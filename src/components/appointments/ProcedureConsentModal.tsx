...
export function ProcedureConsentModal({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center bg-black/70 px-3 py-4">
      <div className="flex max-h-[92vh] w-full max-w-[38rem] flex-col overflow-hidden rounded-lg bg-white shadow-2xl ring-1 ring-black/15">
        <header className="border-b border-neutral-200 bg-white px-5 py-4">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-[0.68rem] font-black uppercase tracking-[0.28em] text-neutral-600">Informed Procedure Consent</p>
            ...
            <button type="button" onClick={onClose} aria-label="Close consent form" className="grid h-10 w-10 shrink-0 place-items-center rounded-full border border-neutral-300 bg-white text-neutral-800 shadow-sm transition hover:border-black hover:text-black">
          ...
          <div className="mt-4 grid grid-cols-2 overflow-hidden rounded-full border border-neutral-300 bg-white p-1 text-xs font-black uppercase tracking-[0.12em] shadow-inner">
            <button type="button" onClick={() => setActiveTab("consent")} className={`rounded-full px-3 py-2 transition ${activeTab === "consent" ? "bg-black text-white ring-1 ring-black" : "text-neutral-600 hover:bg-neutral-100"}`}>
              Patient Consent
            </button>
            <button type="button" onClick={() => setActiveTab("aftercare")} className={`rounded-full px-3 py-2 transition ${activeTab === "aftercare" ? "bg-black text-white ring-1 ring-black" : "text-neutral-600 hover:bg-neutral-100"}`}>
              Post Aftercare
            </button>
          </div>
        </header>
          ...
          {activeTab === "consent" ? (
            <article className="mx-auto rounded-lg border border-neutral-300 bg-white p-4 shadow-sm">
              ...
              <section className="mt-4 rounded-lg border border-neutral-300 bg-white p-3">
                <p className="mx-auto -mt-6 w-max rounded-full bg-black px-4 py-1 text-[0.62rem] font-black uppercase tracking-[0.12em] text-white ring-1 ring-black">Procedure(s) to be performed</p>
                <div className="mt-3 grid grid-cols-2 gap-2 text-[0.72rem] font-semibold text-neutral-700 sm:grid-cols-3">
                  {PROCEDURE_OPTIONS.map((option) => (
                    <span key={option} className="inline-flex items-center gap-1.5">
                      <span className={`grid h-4 w-4 place-items-center rounded-full border text-[0.55rem] ${procedureMatchesOption(procedureName, option) ? "border-black bg-black text-white ring-1 ring-black" : "border-neutral-300 text-transparent"}`}>
              ...
              <div className="mt-3 space-y-2">
                {CONSENT_POINTS.map((point) => (
                  <div key={point} className="grid grid-cols-[1.25rem,1fr] gap-2 text-[0.74rem] leading-5 text-neutral-700">
                    <span className="mt-0.5 grid h-4 w-4 place-items-center rounded bg-black text-[0.65rem] font-black text-white ring-1 ring-black">
                      <FaCheck className="h-2.5 w-2.5" />
                    </span>
                    <p>{point}</p>
                  </div>
                  ...
                  <div className="border-b border-neutral-200 p-3 sm:border-r">
                    <div className="flex items-center justify-between gap-3">
                      ...
                      <div className="flex items-center gap-1.5">
                        ...
                        <button type="button" onClick={clearSignature} disabled={!hasSignature} className="rounded-full border border-neutral-300 px-3 py-1 text-[0.68rem] font-bold text-neutral-700 transition hover:border-black hover:bg-neutral-50 disabled:cursor-not-allowed disabled:opacity-35">Clear</button>
                      </div>
                    </div>
                    <canvas
                      ref={canvasRef}
                    ...
                    <label className="mt-2 block text-[0.64rem] font-black uppercase tracking-[0.12em] text-neutral-500">
                      Printed Name
                      <input value={signatureName} onChange={(event) => setSignatureName(event.target.value)} className="mt-1 w-full border-0 border-b border-neutral-300 bg-transparent px-0 py-1 text-sm font-semibold text-black outline-none focus:border-black" />
                    </label>
            ...
            <article className="mx-auto rounded-lg border border-neutral-300 bg-white p-4 shadow-sm">
              {aftercareGuide ? (
                <>
                  <div className="text-center">
                    <p className="text-[0.68rem] font-black uppercase tracking-[0.24em] text-neutral-600">Post Procedure Aftercare</p>
        ...
        <footer className="flex flex-col gap-3 border-t border-neutral-200 bg-white px-5 py-4 sm:flex-row sm:items-center sm:justify-between">
          ...
          <div className="flex flex-col gap-2 sm:flex-row">
            ...
            {activeTab === "consent" ? (
              <button type="button" onClick={() => setActiveTab("aftercare")} className="rounded-full bg-black px-5 py-2.5 text-sm font-bold text-white ring-1 ring-black transition hover:bg-neutral-800">Next: aftercare</button>
            ) : (
              <button type="button" disabled={!canSave} onClick={saveConsent} className="rounded-full bg-black px-5 py-2.5 text-sm font-bold text-white shadow-sm ring-1 ring-black transition hover:bg-neutral-800 disabled:cursor-not-allowed disabled:bg-neutral-300 disabled:ring-0">Sign & save consent</button>
            )}
          </div>
        </footer>
      </div>
